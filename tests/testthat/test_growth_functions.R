...
test_that("xToZ is vectorized", {
  ...
  # gender <- 'F'
  #
  # xToZ(x, age, gender, chart = 'Fenton2013', measure = 'Wt')
  # xToZ(x, age, gender, chart = 'Fenton2013', measure = 'Wt') %>% pnorm() %>% round(3)

  x      <- c(2946.661, 3155.875, 3360.478, 2835.085, 3049.668, 3239.306) # 50%ile
  age    <- c(      37,       38,       39,       37,       38,       39)
  gender <- c(     'M',      'M',      'M',      'F',      'F',      'F')
  expect_equal(round(xToZ(x, age, gender, chart = 'Fenton2013', measure = 'Wt'), 4), rep(0, length(x)))

  x      <- c(2391.340352, 2595.734374, 2793.553717, 2287.54664, 2486.917178, 2661.786987) # 10%ile
  expect_equal(round(pnorm(xToZ(x, age, gender, chart = 'Fenton2013', measure = 'Wt')), 4), rep(0.1, length(x)))
})
